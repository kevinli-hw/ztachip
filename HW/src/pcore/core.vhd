       ...
       stream_output_r <= (others=>'0');
        ...
        if clock_in'event and clock_in='1' then
            if dp_read_data_flow2_rrrrr=std_logic_vector(to_unsigned(data_flow_direct_c,data_flow_t'length)) then
               ...
               if(dp_read_data_type2_rrrrr = dp_data_type_bfloat_c) then
                  for I in 0 to ddr_vector_width_c/2-1 loop
                     stream_output_r((2*I+2)*data_width_c-1 downto (2*I+0)*data_width_c) <= int2bfloat(output((I+1)*register_width_c-1 downto (I+0)*register_width_c));
                  end loop;
               else
                  for I in 0 to ddr_vector_width_c/2-1 loop
    ...
    if reset_in = '0' then
        ...
        if clock_in'event and clock_in='1' then
            ...
            if (pid_gen_max_c < vector_width_c) and 
              dp_core_read='1' and 
              dp_read_scatter_in/=scatter_none_c then
